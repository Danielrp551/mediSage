...
```
  ...
  └───────────────────────────────┬──────────────────────────────────┘
                                  │  Cloud Task (request FRESCO, CPU asignada, shared-secret)
                                  ▼
  ┌──────────────────────────────────────────────────────────────────┐
  │ POST /api/v1/bots/engine/dispatch   (auth shared-secret, NO RBAC)   │
...
| `scheduling` (#7) | **Tools.** El bot consulta disponibilidad y agenda/cancela citas (`check_availability`/`book_appointment`/`cancel_appointment` → `scheduling.availability`/`scheduling.appointment`/`scheduling.transition`). Estas 3 tools están **registradas y seedeadas** (cierran el loop lead→bot→cita). Vía tools, sin FK directa. | sin FK directa (las tools llaman services de scheduling). |
| `marketing` (#8) | **Tools.** El bot lista promociones elegibles con el descuento ya calculado (`list_eligible_promotions` → `marketing.promotion_usage.eligible_for`) y pasa el `promotion_id` elegido a `book_appointment`. Registrada y seedeada. Vía tools, sin FK directa. | sin FK directa. |
...
- **`tools/{catalog,crm,scheduling,marketing,clock}.py`** — implementaciones de las tools (el `__init__` importa los 5 al boot para poblar `TOOL_REGISTRY`; ver [Tools](#tools-tool_registry)).
...
- La cola entrega un `POST /api/v1/bots/engine/dispatch` con **request fresco y CPU asignada**, autenticado por **shared-secret** (header `X-Bot-Dispatch-Secret` == `BOT_DISPATCH_SECRET`, comparado en tiempo constante con `hmac.compare_digest`; NO RBAC — lo invoca la infra, no un usuario). Se eligió shared-secret **sobre OIDC** porque el servicio es público (Meta/Vercel) → OIDC degradaría a verificación in-app sumando fallas solo-en-prod; OIDC = hardening futuro si el endpoint se separa a un Cloud Run privado. Reintentos + backoff + DLQ + rate-limit configurados **por cola** (una por entorno: qa / prod).
...
- `turn_number: int` NOT NULL — **índice NO único `(conversation_id, turn_number)`** (un turno deja VARIOS eventos —`turn_started` + `turn_completed`/`turn_failed`— con el mismo `turn_number`; el índice agrupa, no restringe a 1 por turno).
...
- **Índices**: `(conversation_id, turn_number)` (NO único — varios eventos por turno) · `(bot_configuration_id, created_on)` · `(event_type, created_on)`.
...
**Registradas en `TOOL_REGISTRY`** (código, vía `@register_tool`; el `__init__` de `tools/` importa los 5 módulos —`catalog`, `crm`, `scheduling`, `marketing`, `clock`— al boot para disparar sus decorators). El dispatcher resuelve la tool por **`code`** (no por `target_service`, que es referencia documental del catálogo). Las tools reciben `(args, ctx: BotInvocationContext, db)` y llaman el service del módulo dueño — **sin** `CurrentAuth` (corren como SYSTEM y validan reglas sistémicas, p.ej. la matriz de transiciones).

| Tool (`code`) | `target_service` | Módulo | Seedeada en `bot_tool` | Qué hace |
|---|---|---|---|---|
| `list_verticals` | `catalog.vertical.list_active` | catalog | no | Lista verticales (read). |
| `list_services_by_vertical` | `catalog.service.list_active` | catalog | no | Lista servicios de una vertical (read). |
| `list_products_by_vertical` | `catalog.product.list_active` | catalog | no | Lista productos de una vertical (read). |
| `resolve_or_create_contact` | `crm.person.find_by_identifier_or_create` | crm | no | Resuelve/crea el contacto (idempotente; advisory lock). |
| `register_lead_note` | `crm.lead_activity.log` | crm | no | Registra una nota en el timeline del lead (`NOTE`, `related_conversation_id=ctx.conversation_id`). |
| `set_lead_status` | `crm.person_lead_status.transition` | crm | no | Transiciona el lead (valida la **matriz** de crm; `requires_confirmation=true`). |
| `check_availability` | `scheduling.availability.compute_available_slots` | scheduling | **sí** | Lista horarios libres de un doctor para un producto en un rango (cómputo on-the-fly). |
| `book_appointment` | `scheduling.appointment.create_appointment` | scheduling | **sí** | Reserva una cita en un slot elegido; `apply_promotion_id` opcional (atómico); `requires_confirmation=true`. |
| `cancel_appointment` | `scheduling.transition.cancel` | scheduling | **sí** | Cancela una cita existente; `requires_confirmation=true`. |
| `list_eligible_promotions` | `marketing.promotion_usage.eligible_for` | marketing | **sí** | Lista promos vigentes que aplican al contacto para un producto, con el descuento ya calculado. |
| `get_current_datetime` | `bots.clock` (sin BD) | clock | no | Ancla temporal: fecha/hora/día en la TZ del negocio (`Settings.BOT_TIMEZONE`); el LLM la llama antes de interpretar fechas. |

> **Qué se seedea en `bot_tool` (`BOT_TOOL_SEED`)**: solo las **4 tools de scheduling + marketing** (`check_availability`, `book_appointment`, `cancel_appointment`, `list_eligible_promotions`) — son las que cierran el loop lead→bot→cita→cliente. Las de **catalog/crm/clock** están registradas en el `TOOL_REGISTRY` (`is_registered=true`) pero **NO** se insertan como filas del catálogo `bot_tool` en el seed; un admin puede crearlas a mano si las quiere asignar a un bot. El M:N `bot_configuration_tool` decide qué tools ve cada bot.
...
| POST | `/engine/dispatch` | **shared-secret (NO RBAC)** | **top-level interno**, target de Cloud Tasks (header `X-Bot-Dispatch-Secret`); body `{conversation_id, input_message_id?}` |
...
> El endpoint `/engine/dispatch` **NO usa RBAC** (lo invoca Cloud Tasks con el shared-secret `X-Bot-Dispatch-Secret`). `BOT_ENGINE_INVOKE` gatea solo el **dispatch manual** de admin (debugging). El turno automático corre con `created_by = SYSTEM` (no hay actor humano).
...
| **F3 — Engine + State + Events + ToolCalls + Cloud Tasks + enganches conversations** | tablas `conversation_bot_state` + `bot_event` + `bot_tool_call`; `EmbeddedBotEngine` (adaptadores `providers/openai.py` + `providers/claude.py`) + `dispatch_turn` + loop tool-calling (≤ `MAX_TOOL_ITERATIONS_PER_TURN`) + `/engine/dispatch` (shared-secret) + `/engine/dispatch-manual` (RBAC) + `app/core/cloud_tasks.py`; **enganches conversations** (`find_or_create_open`→bot + `message.send_bot_outbound`) + provider creds (`--set-secrets` en AMBOS workflows) + **infra Cloud Tasks** (cola qa/prod + secret `BOT_DISPATCH_SECRET`). UI de **depuración** por conversación (state + timeline `BotEvent` + tool calls). **Completa el bot MVP** (preventa/info + captura de lead, embebido OpenAI/Claude). | `0019_bots_engine_state` (22) ✓; down_revision `0018_bots_tools` |
| **F4 — DIFERIDA** | `ExternalBotEngine` + `/engine/external/*`; handoff **automático** (el bot decide pasar a humano); streaming; cost cap **duro**; eval framework. Fuera del MVP. (Las tools de scheduling/marketing ya se cablearon —ver [Tools](#tools-tool_registry)— no esperan a F4.) | (a definir en F4) |
...
1. **Async del turno = Cloud Tasks** (NO `BackgroundTasks`, NO síncrono). El webhook de conversations, tras su pipe síncrono, **encola una Cloud Task**; el endpoint interno `POST /api/v1/bots/engine/dispatch` (shared-secret `X-Bot-Dispatch-Secret`, NO RBAC) corre `dispatch_turn` con CPU asignada. Reintentos + backoff + DLQ + rate-limit por cola. → **[ADR-012](../../decisions/ADR-012-cloud-tasks-bot-dispatch.md)** (nuevo). `app/core/cloud_tasks.py` (cliente lazy + enqueue, molde `secrets.py`/`firestore.py`).
...
4. **Tools = catalog + crm + scheduling + marketing + clock**. El `TOOL_REGISTRY` se puebla de 5 módulos al boot (`tools/__init__.py` importa `catalog`/`crm`/`scheduling`/`marketing`/`clock`): el bot responde info + registra/transiciona leads y notas (crm) + lista catálogo (catalog) + resuelve/crea contacto, **y además** consulta disponibilidad / agenda / cancela citas (scheduling) + lista promociones elegibles (marketing) + se ancla en la fecha actual (clock). **`BOT_TOOL_SEED` seedea solo las 4 tools de scheduling+marketing** (`check_availability`/`book_appointment`/`cancel_appointment`/`list_eligible_promotions`) — cierran el loop lead→bot→cita→cliente; las de catalog/crm/clock quedan registradas (`is_registered=true`) pero no como filas del catálogo `bot_tool`. El nota histórica del diseño 2026-05-28 ("scheduling no existe → diferido") quedó superada cuando scheduling #7 y marketing #8 shippearon.
...
- [ ] **Infra Cloud Tasks** (F3): crear la cola por entorno (qa / prod) + secret `BOT_DISPATCH_SECRET` (Secret Manager → `--set-secrets`, el shared-secret que autentica `/engine/dispatch`) + envs `CLOUD_TASKS_*`. Cliente SDK **lazy** (no romper el boot/smoke sin GCP).
...
- [x] **Tools de scheduling + marketing** (ya cableadas, sin migración): `check_availability`/`book_appointment`/`cancel_appointment` (scheduling) + `list_eligible_promotions` (marketing) registradas en `TOOL_REGISTRY` y seedeadas en `bot_tool` (`BOT_TOOL_SEED`); cierran el loop lead→bot→cita→cliente. El admin las asigna por bot vía el M:N.
