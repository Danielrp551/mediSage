...
Outbound real (`send_outbound`) + `apply_status` (statuses callbacks) llegan en F3. Las
escrituras a Firestore (lib `firebase_admin`, síncronas) se envuelven en `asyncio.to_thread`
...
from app.modules.conversations.enums import ContentType, MessageDirection, SenderType
...
from app.modules.conversations.schemas.message import MessageAttachmentItem, MessageItem
...
async def list_messages(
    db: AsyncSession, conversation_id: str, query: QueryRequest
    ...
    cliente Firestore real-time (onSnapshot)."""
    from app.core import firestore
