...
`services_count` / `products_count` are 0 in phase 1 (Service / Product
models don't exist yet). Phase 2 and 3 will replace the hardcoded zeros
with batch counts joined from those tables.
"""
...
def _to_detail(vertical: Vertical, audit_users: dict[str, User]) -> VerticalDetail:
    return VerticalDetail(**_to_item(vertical, audit_users).model_dump())
...
async def list_paginated(
    ...
    return PaginatedResponse(
        data=PaginatedData(
            items=[_to_item(v, audit_users) for v in items],
            total=total,
...
async def soft_delete(db: AsyncSession, vertical_id: str, *, actor_id: str) -> None:
    ...
    if vertical is None:
        raise NotFoundException("Vertical not found")
    # Phase 2 will add a check for active services here:
