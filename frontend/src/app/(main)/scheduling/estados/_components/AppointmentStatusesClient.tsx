...
} from "@/actions/appointment-status.actions";
...
import type { AppointmentStatusItem } from "@/types/scheduling.types";

import { AppointmentStatusDrawer } from "./AppointmentStatusDrawer";
...
interface Props {
  initialData: ApiPaginated<AppointmentStatusItem>;
}

type DrawerState = { mode: "create" | "edit"; status: AppointmentStatusItem | null } | null;

export function AppointmentStatusesClient({ initialData }: Props) {
  const styles = useStyles();
  const table = useTableQuery<AppointmentStatusItem>({
    queryKey: "scheduling:appointment-statuses",
    fetcher: listAppointmentStatuses,
  ...
  const [deleteTarget, setDeleteTarget] = useState<AppointmentStatusItem | null>(null);
  const [deleteError, setDeleteError] = useState<string | null>(null);
  const [deletePending, setDeletePending] = useState(false);

  const rowActions = useMemo<RowAction<AppointmentStatusItem>[]>(
    () => [
      {
        key: "edit",
        label: "Editar",
        icon: <EditRegular />,
        permissions: ["APPOINTMENT_STATUSES_WRITE"],
  ...
  const handleConfirmDelete = async () => {
    ...
    const result = await deleteAppointmentStatus(deleteTarget.id);
  ...
  return (
    <div className={styles.root}>
      <header className={styles.header}>
        <h1 className={styles.title}>Estados de cita</h1>
        <p className={styles.subtitle}>
          Configura los estados por los que pasa una cita y sus transiciones.
        </p>
      </header>
      ...
      <div className={styles.toolbar}>
        ...
        />
        <PermissionGuard anyOf={["APPOINTMENT_STATUSES_WRITE"]}>
          <Button
            appearance="primary"
            icon={<AddRegular />}
      ...
      <DataTable<AppointmentStatusItem>
        ...
        emptyTitle="Aún no hay estados de cita"
        ...
        columns={[
          ...
          },
          {
            ...
            minWidth: 220,
            onRender: (s) => {
              const hasFlag = s.is_initial || s.is_final || s.is_active_attention;
              ...
              return (
                <span className={styles.flags}>
                  ...
                  {s.is_active_attention ? <Badge appearance="tint">En atención</Badge> : null}
                </span>
              );
            },
          },
