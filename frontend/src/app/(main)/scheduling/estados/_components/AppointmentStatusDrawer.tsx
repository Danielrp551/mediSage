...
import {
  ...
  getAppointmentTransitions,
  listActiveAppointmentStatuses,
  setAppointmentTransitions,
...
import {
  ...
  type AppointmentStatusCreateInput,
...
interface Props {
  ...
  status: AppointmentStatusItem | null;
  onClose: () => void;
}

type FormValues = AppointmentStatusCreateInput & { active?: boolean };

export function AppointmentStatusDrawer({ mode, status, onClose }: Props) {
  const styles = useStyles();
  const { hasPermission } = usePermissions();
  const canWrite = hasPermission("APPOINTMENT_STATUSES_WRITE");
  ...
  const form = useForm<FormValues>({
    resolver: zodResolver(appointmentStatusCreateSchema),
    defaultValues: {
      code: "",
      name: "",
      description: "",
  ...
  const onSubmit = form.handleSubmit((values) => {
    ...
    startTransition(async () => {
      const result =
        statusId !== null
          ? await updateAppointmentStatus(statusId, values)
          : await createAppointmentStatus(values);
      if (!result.ok) {
  ...
  return (
    <Drawer
      open
      onClose={onClose}
      title={isEdit ? "Editar estado de cita" : "Nuevo estado de cita"}
    ...
    >
      ...
      <div className={styles.body}>
        <FormField
          ...
          hint="Mayúsculas, sin espacios (ej. SCHEDULED). No se puede cambiar después."
        >
          <Controller
            control={form.control}
            name="code"
            render={({ field }) => (
              <Input {...field} disabled={!canWrite || isEdit} placeholder="ej. SCHEDULED" />
            )}
          />
        </FormField>
        ...
        <FormField label="Nombre" required error={form.formState.errors.name?.message}>
          <Controller
            ...
            name="name"
            render={({ field }) => (
              <Input {...field} disabled={!canWrite} placeholder="ej. Agendada" />
            )}
          />
        </FormField>
        ...
        <FormField
          label="En atención"
          hint="Marca el estado como “atención en curso” (ej. En atención)."
        >
          <Controller
            control={form.control}
            name="is_active_attention"
            render={({ field }) => (
              <div className={styles.switchRow}>
                <Switch
                  checked={field.value ?? false}
        ...
        {isEdit && statusId ? (
          <>
            ...
            <TransitionMatrixEditor
              statusId={statusId}
              subjectLabel="una cita"
              isFinal={isFinal ?? false}
              canWrite={canWrite}
              fetchOptions={() => listActiveAppointmentStatuses()}
              fetchSelectedIds={async () =>
                (await getAppointmentTransitions(statusId)).to.map((o) => o.id)
              }
              onSave={(toIds) => setAppointmentTransitions(statusId, { to_ids: toIds })}
            />
          </>
        ) : (
          <>
