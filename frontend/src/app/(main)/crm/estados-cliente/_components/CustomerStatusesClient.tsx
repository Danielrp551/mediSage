...
import { deleteCustomerStatus, listCustomerStatuses } from "@/actions/customer-status.actions";
...
import { CustomerStatusDrawer } from "./CustomerStatusDrawer";
...
interface Props {
  initialData: ApiPaginated<CustomerStatusItem>;
}

type DrawerState = { mode: "create" | "edit"; status: CustomerStatusItem | null } | null;

export function CustomerStatusesClient({ initialData }: Props) {
  const styles = useStyles();
  const table = useTableQuery<CustomerStatusItem>({
    queryKey: "crm:customer-statuses",
    fetcher: listCustomerStatuses,
  ...
  const [deleteTarget, setDeleteTarget] = useState<CustomerStatusItem | null>(null);
  const [deleteError, setDeleteError] = useState<string | null>(null);
  const [deletePending, setDeletePending] = useState(false);

  const rowActions = useMemo<RowAction<CustomerStatusItem>[]>(
    () => [
      {
        key: "edit",
        label: "Editar",
        icon: <EditRegular />,
        permissions: ["CUSTOMER_STATUSES_WRITE"],
  ...
  const handleConfirmDelete = async () => {
    ...
    const result = await deleteCustomerStatus(deleteTarget.id);
  ...
  return (
    <div className={styles.root}>
      <header className={styles.header}>
        <h1 className={styles.title}>Estados de cliente</h1>
        <p className={styles.subtitle}>
          Configura los estados por los que pasa un cliente y sus transiciones.
        </p>
      </header>
      ...
      <div className={styles.toolbar}>
        ...
        />
        <PermissionGuard anyOf={["CUSTOMER_STATUSES_WRITE"]}>
          <Button
            appearance="primary"
            icon={<AddRegular />}
      ...
      <DataTable<CustomerStatusItem>
        ...
        emptyTitle="Aún no hay estados de cliente"
        ...
        columns={[
          ...
          },
          {
            ...
            minWidth: 160,
            onRender: (s) => {
              const hasFlag = s.is_initial || s.is_final;
      ...
      <ConfirmDialog
        ...
        title="¿Eliminar estado de cliente?"
        description={
          deleteError
            ? deleteError
            : deleteTarget
